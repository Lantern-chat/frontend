...
function decode(enc: Uint8Array): ArrayBuffer {
    let out = new ArrayBuffer(enc.length / 5 * 4);

    let pos = 0, w = 0, view = new DataView(out);
    while(pos < enc.length) {
        let block_num = 0, next_pos = pos + 5;
        enc.slice(pos, next_pos).forEach(c => {
            if(c <= 32 || c > 127) {
                throw "Invalid Z85 Byte!";
            }
            let k = BYTE_OFFSETS[c - 32];
            if(k == -1) {
                throw "Invalid Z85 Byte!";
            }
            block_num = block_num * 85 + k;
        });
